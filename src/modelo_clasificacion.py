...
class ModeloClasificacion(ModeloBase):
    ...
    Atributos de clase:
        NOMBRES_CLASES (list): Etiquetas legibles de cada nivel de saturación.
        COLORES_CLASES (list): Paleta de colores para visualización.
        LINEA_MAPPING (dict): Mapeo de linea_encoded → nombre de línea del STC.
        MES_MAPPING (dict): Mapeo de mes_encoded → nombre de mes (orden alfabético).
        DEFAULT_PARAM_DISTRIBUTIONS (dict): Espacio de hiperparámetros por defecto
            para RandomizedSearchCV.

    Atributos de instancia:
        busqueda_ (RandomizedSearchCV): Objeto de búsqueda de hiperparámetros,
            disponible tras llamar a entrenar().
    """
    ...
    LINEA_MAPPING = {
         0: 'Línea 1',  1: 'Línea 12',  2: 'Línea 2',   3: 'Línea 3',
         4: 'Línea 4',  5: 'Línea 5',   6: 'Línea 6',   7: 'Línea 7',
         8: 'Línea 8',  9: 'Línea 9',  10: 'Línea A',  11: 'Línea B'
    }
    MES_MAPPING = {
         0: 'Abril',      1: 'Agosto',    2: 'Diciembre',  3: 'Enero',
         4: 'Febrero',    5: 'Julio',     6: 'Junio',       7: 'Marzo',
         8: 'Mayo',       9: 'Noviembre', 10: 'Octubre',   11: 'Septiembre'
    }
    ...
    def entrenar(self, X_train: pd.DataFrame, y_train: pd.Series,
        ...
        Args:
            ...
            param_distributions (Dict, optional): Diccionario de hiperparámetros.
                Si es None, usa DEFAULT_PARAM_DISTRIBUTIONS.
    ...
    def predecir(self, X: pd.DataFrame) -> np.ndarray:
        ...
        Args:
            X (pd.DataFrame): Datos de entrada para realizar predicciones
                (debe contener columnas numéricas).

        Returns:
            np.ndarray: Predicciones generadas por el modelo
                (valores enteros 0-3 correspondientes a las clases).
        """
    ...
    def evaluar(self, X_test: pd.DataFrame, y_test: pd.Series) -> Dict:
        ...
        Métricas calculadas:
            - Accuracy, Precision, Recall, F1-Score (por clase, macro y ponderado).
            - Matriz de Confusión.
        ...
        return {
            ...
            'report_texto'        : classification_report(
                                        y_test, y_pred, target_names=self.NOMBRES_CLASES)
        }

    def accuracy_entrenamiento(self, X_train: pd.DataFrame, y_train: pd.Series) -> float:
        """
        Calcula la accuracy del modelo en el conjunto de entrenamiento para
        diagnosticar sobreajuste (gap entrenamiento vs prueba).

        Args:
            X_train (pd.DataFrame): Datos de entrenamiento.
            y_train (pd.Series): Etiquetas verdaderas de entrenamiento.

        Returns:
            float: Accuracy del modelo sobre los datos de entrenamiento.
        ...
        return accuracy_score(y_train, self.predecir(X_train))
    ...
    def importancia_variables(self) -> pd.DataFrame:
        ...
        Returns:
            pd.DataFrame: DataFrame con nombres de variables e importancia,
    ...
    def resultados_busqueda(self) -> pd.DataFrame:
        """
        Devuelve un DataFrame con los resultados de todas las combinaciones de
        hiperparámetros evaluadas en RandomizedSearchCV.

        Returns:
            pd.DataFrame: DataFrame con columnas de hiperparámetros y métricas
                de evaluación para cada combinación probada.

        Raises:
            ValueError: Si el modelo no ha sido entrenado con búsqueda de hiperparámetros.
        """
    ...
    def baseline_dummy(self, X_train: pd.DataFrame, y_train: pd.Series,
                       X_test: pd.DataFrame, y_test: pd.Series) -> float:
        """
        Calcula la métrica de referencia utilizando un clasificador Dummy que siempre
        predice la clase más frecuente, aprendida desde el conjunto de entrenamiento.

        Args:
            X_train (pd.DataFrame): Datos de entrenamiento para aprender la clase
                mayoritaria real del sistema.
