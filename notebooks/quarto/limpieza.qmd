...
df_modelo['estacion_encoded'] = le_estacion.fit_transform(df_modelo['estacion'])
...
anios_dataset = df_modelo['fecha'].dt.year.unique()
mx_holidays = holidays.country_holidays('MX', years=anios_dataset)
df_modelo['es_festivo'] = df_modelo['fecha'].isin(mx_holidays).astype(int)

df_modelo['es_quincena'] = df_modelo['fecha'].dt.day.isin([14, 15, 30, 31]).astype(int)

df_modelo['lluvia_historica'] = df_modelo['fecha'].dt.month.isin([5, 6, 7, 8, 9, 10]).astype(int)
