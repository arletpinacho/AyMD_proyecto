...
## Diseño e implementación con POO: clase `MetroSaturacionModel`

El código fuente del modelo está organizado mediante **Programación Orientada a Objetos** en la clase `MetroSaturacionModel`. Esta clase encapsula de forma cohesiva todas las responsabilidades del ciclo de vida del modelo supervisado, separando la **lógica del modelo** (entrenamiento, predicción, evaluación, persistencia) de la **lógica de presentación** (visualizaciones y reportes del notebook).

El principio de diseño central es el **Patrón Pipeline** de scikit-learn: se eligió este patrón porque el procesamiento de datos ocurre en **etapas secuenciales bien definidas** (normalización (`StandardScaler`) → clasificación (`RandomForestClassifier`)), lo que permite encapsular ambas transformaciones en un único objeto cohesivo, serializable y reproducible. Al serializar el Pipeline completo en disco, el escalador guarda los parámetros aprendidos en entrenamiento, garantizando que las predicciones en producción se realicen con la misma normalización sin riesgo de inconsistencias.
...
- **Entrenamiento**: `ajustar_hiperparametros()` — búsqueda con `RandomizedSearchCV` + `StratifiedKFold`.
...
- **Persistencia**: `guardar()` y `cargar()` — serialización y deserialización con joblib.
...
La clase `MetroSaturacionModel` queda instanciada con `seed=42`. Los métodos públicos disponibles son: `ajustar_hiperparametros()`, `cargar()` (classmethod), `evaluar()`, `guardar()`, `importancia_variables()`, `predecir()`, `predecir_proba()` y `resultados_busqueda()`.
...
```{python}
dummy_acc = modelo.baseline_dummy(X_test, y_test)
clase_mas_frecuente = NOMBRES_CLASES[dummy.predict(X_test[:1])[0]]

print("Clasificador Trivial (DummyClassifier — most_frequent)")
print(f"\n  Accuracy en prueba         : {dummy_acc:.4f}  ({dummy_acc*100:.2f}%)")
...
for param, valores in param_distributions.items():
    print(f"  {param:<47}: {valores}")

print("\nIniciando búsqueda de hiperparámetros...")
print("─" * 60)

# CAMBIO CLAVE: entrenar() en lugar de ajustar_hiperparametros()
...
```{python}
print("Evaluación del modelo supervisado con MetroSaturacionModel.evaluar()")

metricas = modelo.evaluar(X_test, y_test)
acc_train = accuracy_score(y_train, modelo.predecir(X_train))
...
El Pipeline completo (que incluye el `StandardScaler` con sus parámetros de normalización ajustados sobre `X_train` y el `RandomForestClassifier` con los hiperparámetros óptimos) se serializa en disco con `joblib` a través del método `guardar()` de la clase `MetroSaturacionModel`.
...
print("Persistencia del modelo con MetroSaturacionModel.guardar()")
...
La siguiente celda es **funcionalmente independiente** del entrenamiento: utiliza únicamente el método de clase `MetroSaturacionModel.cargar()` para reconstruir el modelo desde disco y realizar predicciones sin acceso a las variables de entrenamiento originales. Esto simula una sesión de inferencia en producción.
...
print("(Celda independiente: simula inferencia sin re-entrenamiento)\n")
